...
def heilbronn_triangle(n,m):
    ...
    point_in_rectangle = model.addVars(n, n, vtype=GRB.BINARY, name="point_in_square")
    ...
    for j in range(m):
        model.addConstr(quicksum(point_in_square[i,j, k] for k in range(n)) <= 2, f"Rectangle_{j}_capacity")
    ...
    rectangle_size = 1.0 / m
    for k in range(n):
        for j in range(m):
            model.addConstr(point_in_rectangle[j, k] * (y[k] - j * rectangle_size) >= 0, f"link_y_lb_{j}_{k}")
            model.addConstr(point_in_rectangle[j, k] * (y[k] - (j + 1) * rectangle_size) <= 0, f"link_y_ub_{j}_{k}")
...
n = int(input())
m = int(input())
optimal_z ,optimal_x, optimal_y, optimize_time = heilbronn_triangle(n,m)
