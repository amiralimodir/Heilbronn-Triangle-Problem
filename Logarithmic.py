...
def heilbronn_triangle(n):
    ...
    r = model.addVars(n, 5, vtype=GRB.BINARY, name="r")
    e = model.addVars(n, vtype=GRB.CONTINUOUS, name="e", lb=0, ub=0.03125)
    r2 = model.addVars(n, 5, vtype=GRB.BINARY, name="r2")
    e2 = model.addVars(n, vtype=GRB.CONTINUOUS, name="e2", lb=0, ub=0.03125)
    x = model.addVars(n, vtype=GRB.CONTINUOUS, name="x", lb=0, ub=1)
    y = model.addVars(n, vtype=GRB.CONTINUOUS, name="y", lb=0, ub=1)
    
    for i in range(n):
        model.addConstr(x[i] == 0.5 * r[i, 0] + 0.25 * r[i, 1] + 0.125 * r[i, 2] + 0.0625 * r[i, 3] + 0.03125 * r[i, 4] + e[i], name=f"x_constr_{i}")
        model.addConstr(y[i] == 0.5 * r2[i, 0] + 0.25 * r2[i, 1] + 0.125 * r2[i, 2] + 0.0625 * r2[i, 3] + 0.03125 * r2[i, 4] + e2[i], name=f"y_constr_{i}")
